...
Call depositETH on the real Optimism L1StandardBridge

Receive an unexpected external callback during processing (via fallback)

Use that callback window to perform cross-layer reentrancy

Ultimately double-spend ETH across L1 and L2
...
Funds an EOA with 100 ETH

Calls the attacker → which calls the bridge
...
Captures behavior through verbose call traces (-vvv)
